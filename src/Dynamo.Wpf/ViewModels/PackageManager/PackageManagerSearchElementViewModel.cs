...
namespace Dynamo.PackageManager.ViewModels
{
    public class PackageManagerSearchElementViewModel : BrowserItemViewModel, IEquatable<PackageManagerSearchElementViewModel>
    {
            ...
            : base(element)
        {
            ...
            this.ToggleIsExpandedCommand = new RelayCommand(() => this.SearchElementModel.IsExpanded = !this.SearchElementModel.IsExpanded);

            this.DownloadLatestCommand = new RelayCommand(
                () => OnRequestDownload(false),
                () => !SearchElementModel.IsDeprecated && CanInstall);
            this.DownloadLatestToCustomPathCommand = new RelayCommand(() => OnRequestDownload(true));

            this.UpvoteCommand = new RelayCommand(SearchElementModel.Upvote, () => canLogin);

            this.VisitSiteCommand =
                new RelayCommand(() => GoToUrl(FormatUrl(SearchElementModel.SiteUrl)), () => !String.IsNullOrEmpty(SearchElementModel.SiteUrl));
            this.VisitRepositoryCommand =
                new RelayCommand(() => GoToUrl(FormatUrl(SearchElementModel.RepositoryUrl)), () => !String.IsNullOrEmpty(SearchElementModel.RepositoryUrl));
        }
        ...
        /// </summary>
        public List<Tuple<PackageVersion, RelayCommand<object>>> Versions
        {
            get
            {
                return
                    SearchElementModel.Header.versions.Select(
                        x => new Tuple<PackageVersion, RelayCommand<object>>(
                            x, new RelayCommand<object>((p) => OnRequestDownload(x, p.Equals("true")))
                        )).Reverse().ToList();
            }
        }
