...
namespace Dynamo.ViewModels
{
    ...
    public partial class ConnectorViewModel : ViewModelBase
    {
        ...
        {
            BreakConnectionCommand = new RelayCommand<object>(BreakConnectionCommandExecute);
            ShowhideConnectorCommand = new RelayCommand<object>(HideConnectorCommandExecute);
            SelectConnectedCommand = new RelayCommand<object>(SelectConnectedCommandExecute);
            MouseHoverCommand = new RelayCommand<object>(MouseHoverCommandExecute, CanRunMouseHover);
            MouseUnhoverCommand = new RelayCommand<object>(MouseUnhoverCommandExecute, CanRunMouseUnhover);
            PinConnectorCommand = new RelayCommand<object>(PinConnectorCommandExecute);
            InstantiateContextMenuCommand = new RelayCommand<object>(InstantiateContextMenuCommandExecute, CanInstantiateContextMenu);
            GoToStartNodeCommand = new RelayCommand<object>(GoToStartNodeCommandExecute);
            GoToEndNodeCommand = new RelayCommand<object>(GoToEndNodeCommandExecute);
        }
        ...
        public void Redraw(object parameter)
        {
            ...
            var startPort = ActiveStartPort ?? model?.Start;
            var endPort=model?.End;
            if (startPort != null)
            {
                if (startPort.Alignment == PortAlignment.Left || startPort.Alignment == PortAlignment.Right)
                {
                    CurvePoint1 = new Point(CurvePoint0.X + dx, CurvePoint0.Y);
                }
                else
